...
## 简介
将抽象部分与它的实现部分分离， 使它们都可以独立地变化。如有一个附魔武器的接口，武器能够挥舞，同时武器也能有魔法攻击，如果将魔法动作和武器本身的动作都定义在接口方法中，则针对每种武器的每种魔法，都要有一个具体的实现类，显然是不应该的。为了武器和魔法动作能够分别变化，将魔法动作抽象为一个接口，武器接口只需要能够访问到魔法接口，就能实现武器和魔法动作的解耦，如武器和魔法的接口定义：
```java
/**
 * 武器
 */
