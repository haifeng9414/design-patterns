...
## 目的
为其他对象提供一种代理以控制对这个对象的访问，代码形式上可能代理模式和装饰器模式很像，但是从语意上讲，代理模式的目标是控制对被代理对象的访问，代理类附加的是跟原始类无关的功能，如监控、统计、鉴权、限流、事务、幂等、日志等，而装饰模式是给原对象增加额外功能，装饰器类附加的是跟原始类相关的增强功能，如JDK中`BufferedInputStream`的构造函数接受一个`InputStream`对象，为`InputStream`对象附加了一个读取数据到缓冲区的功能。
