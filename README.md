...
## 其他
- 编写代码时，不要还没开始写就先想着套用某个设计模式。设计模式简单来说是为了保证高内聚、低耦合，也就是利用更好的代码结构将一大坨代码拆分成职责更单一的小类。创建型模式是将创建和使用代码解耦，结构型模式是将不同的功能代码解耦，行为型模式是将不同的行为代码解耦，而解耦的主要目的是应对代码的复杂性。在代码设计和实现之初就强加各种设计模式，可能会导致项目的复杂度提高和可读性的降低。正确的做法是采用持续重构的开发方法。持续重构不仅仅是保证代码质量的重要手段，也是避免过度设计的有效方法。在真正有痛点的时候（可读性、可扩展性、可维护性不好），再去考虑用设计模式来解决，而不是一开始就为不一定实现的未来需求而应用设计模式。
